...
pub fn insert_call_into_fn_body(fn_body: &mut syn::Block, info: &FakeableFnInfo) {
    let fake_access_fn_name = &info.fake_access_fn_name;
    let fn_param_idents = &info.fn_param_idents;
    let fn_generic_idents = info.generic_info
        .as_ref()
    ...
    let call_expr =
        quote! {
        #[cfg(test)]
        if #fake_access_fn_name::<#(#fn_generic_idents),*>().is_set() {
            let impl_fn = #fake_access_fn_name::<#(#fn_generic_idents),*>().get();

//! Name derivation for a free function's fake.
//!
//! Kept together so the module, store, accessor and interface names generated for a single
//! function are guaranteed to agree on what they call each other.
...
mod tests {
    ...
    fn test_build_module_name() {
        ...
        assert_eq!(build_module_name(&fn_name).to_string(), "get_user_fake_module");
    }

    #[test]
    fn test_build_store_name() {
        let fn_name: syn::Ident = syn::parse_quote!(get_user);
        assert_eq!(build_store_name(&fn_name).to_string(), "GET_USER_FAKE_STORE");
    }

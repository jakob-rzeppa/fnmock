...
mod tests {
    ...
    #[test]
    fn test_non_generic() {
        let interface_struct_name: syn::Ident = parse_quote!(MyFunctionInterface);

        let res = build_interface_struct(&interface_struct_name, None);

        let expected = quote! {
            pub struct MyFunctionInterface;
        };
    ...
    fn test_generic_with_single_param() {
        ...
        let expected = quote! {
            pub struct MyFunctionInterface<T> {
    ...
    fn test_generic_with_multiple_params_excludes_const_generics_from_marker() {
        ...
        let expected = quote! {
            pub struct MyFunctionInterface<T, const C: u32> {
                _marker: ::std::marker::PhantomData<(T)>,
            }
        };
    ...
    fn test_generic_with_empty_idents_slice() {
        ...
        let expected = quote! {
            pub struct MyFunctionInterface<const C: u32> {

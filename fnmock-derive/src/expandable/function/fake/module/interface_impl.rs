...
mod tests {
    ...
    fn test_non_generic() {
        ...
        let expected = quote! {
            impl MyFunctionInterface {
                ...
                pub fn is_set(&self) -> bool {
                    MY_FUNCTION_STORE.with(|store| store.borrow().is_set())
                }
            }
        };
    ...
    fn test_generic_with_single_param() {
        ...
        let expected = quote! {
            impl<T> MyFunctionInterface<T> {
                pub fn setup(&self, function: impl Fn(i32) -> bool + 'static) {
                    MY_FUNCTION_STORE.with_borrow_mut(|fake| {
    ...
    fn test_generic_with_multiple_params_and_keys() {
        ...
        let expected = quote! {
            impl<T, const C: u32> MyFunctionInterface<T, C> {

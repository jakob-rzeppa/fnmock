...
use crate::item_info::{
    impl_block::generics::{extract_method_generic_info, extract_struct_generic_info},
...
pub struct ImplBlockInfo {
    ...
    pub struct_generic_info: Option<StructGenericInfo>,

    /// One entry per method in the impl block, in the order the methods appear in the block.
    pub functions: Vec<ImplMethodInfo>,
}

/// The generic parameters of an impl block's struct, independent of any one method.
pub struct StructGenericInfo {
    /// The parameters including their bounds (e.g. `S: Display + 'static`), for redeclaring them
    /// on generated items.
    pub generic_params: Vec<syn::GenericParam>,

    /// Just the parameters' identifiers (e.g. `S`), for instantiating generated items.
    pub idents: Vec<syn::Ident>,

    /// The `GenericKeyPart` expressions that key a store by the struct's generics, in parameter
    /// order.
...
pub struct ImplMethodInfo {
    ...
    /// The method's own generic parameters, or `None` if it has none. The struct's generic
    /// parameters (if any) live on the enclosing [`ItemImplInfo::struct_generic_info`] instead —
    /// see its doc comment for why they're kept apart rather than combined here.
    pub generic_info: Option<ImplMethodGenericInfo>,
}

/// The generic parameters of a single method, independent of its struct's.
pub struct ImplMethodGenericInfo {
    /// The parameters including their bounds (e.g. `M: Display + 'static`), for redeclaring them
    /// on generated items.
    pub generic_params: Vec<syn::GenericParam>,

    /// Just the parameters' identifiers (e.g. `M`), for instantiating generated items.
    pub idents: Vec<syn::Ident>,

    /// The `GenericKeyPart` expressions that key a store by the method's generics, in parameter
    /// order.
    pub generic_keys: Vec<syn::Expr>,
}
...
fn extract_single_item_impl_info_for_method(
    ...
    let generic_info = extract_method_generic_info(method)?;
...
mod tests {
    ...
    fn test_struct_generics_are_extracted_once_not_once_per_method() {
        ...
        let Some(struct_generic_info) = &info.struct_generic_info else {
            panic!("expected struct_generic_info to be Some for a generic struct");
        };
        assert_eq!(
            struct_generic_info
                .idents
    ...
    #[test]
    fn test_method_only_generics_do_not_populate_struct_generic_info() {
        let item_impl: syn::ItemImpl = syn::parse_quote! {
            impl Foo {
                fn bar<M>(&self, x: M) {}
            }
        ...
        assert!(
            info.struct_generic_info.is_none(),
            "expected struct_generic_info to be None when only the method is generic"
        );
        assert!(
            info.functions[0].generic_info.is_some(),
            "expected the method's own generic_info to be Some"
        );
    }
}

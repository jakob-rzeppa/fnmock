...
mod tests {
    ...
    fn test_merge_where_bounds_into_type_params_merges_bounds() {
        let function: syn::ItemFn =
            syn::parse_quote!(
            fn example<T>() where T: Clone + 'static {}
        );
        ...
        let expected_type_params: Vec<syn::GenericParam> = vec![
            syn::parse_quote!(T: Clone + 'static)
        ];
        assert_eq!(
    ...
    fn test_merge_where_bounds_into_type_params_ignores_non_parameter_bounds() {
        let function: syn::ItemFn =
            syn::parse_quote!(
            fn example<T>() where Vec<T>: Clone {}
        );
        let generics = function.sig.generics;
    ...
    fn test_merge_where_bounds_into_type_params_ignores_duplicate_bounds() {
        let function: syn::ItemFn =
            syn::parse_quote!(
            fn example<T: Clone>() where T: Clone {}
        );
        let generics = function.sig.generics;

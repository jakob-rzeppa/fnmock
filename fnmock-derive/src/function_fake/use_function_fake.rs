...
/// Processes a use statement and generates conditional imports for fake versions.
///
/// This is the main entry point for the use_function_fake attribute macro. It analyzes
/// a use statement and generates conditional compilation attributes that:
/// - Import the original functions in production builds
/// - Import fake versions (with `_fake` suffix) aliased to original names in test builds
///
/// # Arguments
///
...
pub(crate) fn process_use_function_fake(input: syn::ItemUse) -> syn::Result<proc_macro2::TokenStream> {
    ...
    Ok(
        ...
        if function_mappings.len() == 1 {
            let (fn_name, fake_fn_name) = &function_mappings[0];
            generate_single_import(&input, module_path, fn_name, fake_fn_name)
        } else {
...
/// Generates the expanded code for a single function import with fake version.
///
/// Creates conditional compilation attributes that import the original function
/// in production builds and the fake version (aliased to the original name) in test builds.
///
/// # Arguments
///
...
/// * `fake_fn_name` - The fake function name (with `_fake` suffix)
///
/// # Returns
///
...
/// #[cfg(test)]
/// use module::path::function_fake as function;
/// ```
pub(crate) fn generate_single_import(
    ...
    quote! {
        ...
        #[cfg(test)]
        use #module_path::#fake_fn_name as #fn_name;
    }
}

/// Generates the expanded code for multiple function imports with fake versions.
///
/// Creates conditional compilation attributes that import the original functions
/// in production builds and the fake versions (aliased to the original names) in test builds.
///
/// # Arguments
///
/// * `input` - The original use statement
/// * `module_path` - The module path tokens (empty if importing from current module)
/// * `function_mappings` - Vector of (original_name, fake_name) tuples
///
/// # Returns
///
/// Token stream containing:
...
/// use original::statement;
/// #[cfg(test)]
/// use module::path::{fn1_fake as fn1, fn2_fake as fn2};
/// ```
pub(crate) fn generate_multiple_imports(
    ...
    let fake_alias_mappings: Vec<_> = function_mappings
        .iter()
        .map(|(fn_name, fake_fn_name)| {
            quote! { #fake_fn_name as #fn_name }
        })
        .collect();
    ...
    quote! {
        ...
        #[cfg(test)]
        use #module_path::{#(#fake_alias_mappings),*};
    }
}

...
mod mock {
    ...
    #[test]
    fn test_assert_ignores_another_instantiations_unfulfilled_expectation() {
        let mock_string = assert_scoped_to_instantiation_mock::<String>();
        let mock_i32 = assert_scoped_to_instantiation_mock::<i32>();

        mock_string.expect(fnmock::predicate::always()).once();
        mock_i32.expect(fnmock::predicate::always()).once();

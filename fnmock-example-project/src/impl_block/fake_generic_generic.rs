...
impl<T: 'static> UserRepository<T> {
    ...
    pub fn get_user<I: Display + 'static>(&self, user_id: I) -> Option<String> {
        #[cfg(test)]
        if UserRepositoryFake::get_user_fake::<T>::is_set_for::<I>() {
            let impl_fn = UserRepositoryFake::get_user_fake::<T>::get_for::<I>();
...
pub(crate) mod UserRepositoryFake {
    ...
    thread_local! {
        static GET_USER_FAKE: std::cell::RefCell<GenericFakeStore<2>> = std::cell::RefCell::new(
    ...
    impl<T: 'static> get_user_fake<T> {
        pub(crate) fn setup<I: Display + 'static>(
            function: fn(&UserRepository<T>, I) -> Option<String>
        ) {
            let generic_types = [TypeId::of::<T>(), TypeId::of::<I>()];
        ...
        pub(crate) fn clear_for<I: Display + 'static>() {
            let generic_types = [TypeId::of::<T>(), TypeId::of::<I>()];
        ...
        pub(crate) fn is_set_for<I: Display + 'static>() -> bool {
            let generic_types = [TypeId::of::<T>(), TypeId::of::<I>()];

            GET_USER_FAKE.with_borrow(|fake| { fake.is_set_for(generic_types) })
        }

        pub(crate) fn get_for<I: Display + 'static>() -> Rc<
            fn(&UserRepository<T>, I) -> Option<String>
        > {
            let generic_types = [TypeId::of::<T>(), TypeId::of::<I>()];

            GET_USER_FAKE.with_borrow(|fake| {
                fake.get_for::<fn(&UserRepository<T>, I) -> Option<String>>(generic_types)
            })
        }
    }
}
...
mod tests {
    ...
    fn test_handle_user_with_fake_repo() {
        UserRepositoryFake::get_user_fake::<String>::setup::<u32>(|_, user_id| {

...
[unsupported/compile_fail/](fnmock-tests/src/fake/unsupported/compile_fail/).
...
own store. The accessor is an associated function (`Type::method_fake()`; see
[FEATURES.md](FEATURES.md#the-accessor) for the exact call shapes).
...
The two sets of generic arguments are split across the positions they belong to — struct generics
on the type, method generics on the accessor (`GenericCombined::<String>::combine_fake::<i32>()`).
